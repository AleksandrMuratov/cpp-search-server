...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words)
        : stop_words_(MakeUniqueNonEmptyStringsAndCheckValidity(stop_words)) {
    }
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        if (!IsValidId(document_id)) throw invalid_argument("Not valid id of document"s);
        if (documents_.count(document_id)) throw invalid_argument("This document has already been added to the database"s);
...
private:
    ...
    template <typename StringContainer>
    static set<string> MakeUniqueNonEmptyStringsAndCheckValidity(const StringContainer& strings) {
